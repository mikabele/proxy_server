...
class ProxyServer:
    ...
    __cached_requests: dict[str, deque] = None
    ...
    async def get_request_params(self, request_str: str) -> RequestParams:
    ...
    async def handle_request(self, request: str) -> RequestResult:
    ...
    async def write_result(self, writer: asyncio.StreamWriter, result: RequestResult) -> None:
        sending_str = await self.get_sending_str(result)
        writer.write(bytearray(sending_str, encoding="UTF-8"))
        await writer.drain()
        writer.close()

    async def get_sending_str(self, requested_result: RequestResult) -> str:
    ...
    async def cache_request(self, request: str, func: str, args: dict, request_info: RequestResult) -> None:
    ...
    async def get_cached_request(self, request: str, func: str, args: dict) -> RequestResult:
        func_and_args_str = func + str(args)
